...
object MyExcel extends Excel {
  implicit object NodeSeqFormatter extends SimpleFormatter[NodeSeq] {
    def apply(value: Option[NodeSeq]) =
      implicitly[Formatter[String]].apply(value.map(_.toString))
  }
}
...
class ExcelSpec extends Specification {
  ...
  "a Excel output" should {
    "produce Excel output" in {
      ...
      Excel(() => new MockWorkbook(new HSSFWorkbook())) { ctx =>
        val writer1 = MyExcel
          .writer(Sheet1.columns.columns)
          .toWorkbook(ctx, name = Some("sheet one"))
        writer1.start()
        ...
        writer1.finish()
        Sheet2.columns.write(MyExcel)(
          _.toWorkbook(ctx, name = Some("sheet two")))(Purchases.*.iterator)
      }.workbook.write(out)
      out.flush()
      out.close()
      println(file)
      ...
      Purchases.*.zip(rows1).foreach {
        case (Purchase(UselessItem(_name, _price, _, _),
                       _date,
                       PretentiousPurveyor(_, _location),
                       _quantity),
              row) =>
          val name :: _ :: total :: _ :: location :: date :: _ :: Nil =
            row.cells()
          name.value() must beSome.which {
            case x: String => x.toLowerCase == _name
          }
      ...
      Purchases.*.zip(rows2).foreach {
        case (Purchase(UselessItem(_name, _, tags, _), _, _, _), row) =>
          val name :: tag_foo :: tag_bar :: tag_baz :: tag_quux :: Nil =
            row.cells()
          name.value() must beSome.which {
            case x: String => x.toLowerCase == _name
          }
