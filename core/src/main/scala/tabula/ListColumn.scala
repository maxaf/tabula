...
/** Wrapper around a List[Column] that allows a single column to
  ...
  */
abstract class ListColumn[F, T, C, Cols](
    val underlying: List[Column[F, T, C]])(
    implicit mf: Manifest[F],
    cz: Cellulizer[List[ColumnAndCell[F, T, C]], List[C]])
    extends Column[F, List[ColumnAndCell[F, T, C]], List[C]](x =>
      Some(underlying.map(_(x))))
